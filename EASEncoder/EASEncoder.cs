...
namespace EASEncoder
{
    public static class EASEncoder
    {
        ...
        private static byte[] GenerateVoiceAnnouncement(string announcement, string speechSynthesizerVoice)
        {
            var synthesizer = new SpeechSynthesizer();
            var waveStream = new MemoryStream();
            var firstOrDefault = synthesizer.GetInstalledVoices()
                .FirstOrDefault(x => x.VoiceInfo.Name.ToUpper().Contains(speechSynthesizerVoice));
            if (firstOrDefault != null)
                synthesizer.SelectVoice(
                    firstOrDefault
                        .VoiceInfo.Name);
